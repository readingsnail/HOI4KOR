        ...
        if map.get('status') :
            ...
            if map['status'] in ['clean', 'novalue', 'nokey'] :
                if map.get('chain') : ## if map['chain'] is '' or not exist
                    tempstr = map['key']
                    tempstr = tempstr + ' ' + map['value']
                    ##return {'status' : map['status'], 'line' : (map['key'] + chaindelim + map['chain'] + valdelim + self.wrap(tempstr, wletter) ) }
                    return {'status' : map['status'], 'line' : (map['key'] + chaindelim + map['chain'] + valdelim + '"' + tempstr +'"' ) }
                else :
